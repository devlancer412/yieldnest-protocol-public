...
import {RewardsReceiver} from "contracts/RewardsReceiver.sol";
...
contract RewardsDistributor is AccessControlUpgradeable, RewardsDistributorEvents {
   ...
   */
  function processRewards() external assertBalanceUnchanged {
  ...
  ) external onlyRole(REWARDS_ADMIN_ROLE) notZeroAddress(newReceiver) {
  ...
  function setFeesBasisPoints(uint16 newFeesBasisPoints) external onlyRole(REWARDS_ADMIN_ROLE) {
