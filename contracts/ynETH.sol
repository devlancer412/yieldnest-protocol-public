...
contract ynETH is IynETH, ynBase, IYnETHEvents {
  ...
  function withdrawETH(uint256 ethAmount) public override onlyStakingNodesManager {
  ...
  modifier onlyStakingNodesManager() {
    if (msg.sender != address(stakingNodesManager)) {
      revert CallerNotStakingNodeManager(address(stakingNodesManager), msg.sender);
    }
    _;
  }
