...
contract YieldNestOracle is AccessControlUpgradeable, IYieldNestOracleEvents {
  ...
  error ArraysLengthMismatch(
    ...
    uint256 priceFeedAddressesLength,
    uint256 maxAgesLength
  );
  ...
  function initialize(
    Init memory init
  ) external notZeroAddress(init.admin) notZeroAddress(init.oracleManager) initializer {
    __AccessControl_init();
    _grantRole(DEFAULT_ADMIN_ROLE, init.admin);
    _grantRole(ORACLE_MANAGER_ROLE, init.oracleManager);

    if (
      init.assets.length != init.priceFeedAddresses.length ||
      init.assets.length != init.maxAges.length
    ) {
      revert ArraysLengthMismatch({
        assetsLength: init.assets.length,
        priceFeedAddressesLength: init.priceFeedAddresses.length,
        maxAgesLength: init.maxAges.length
      });
    }
