...
import {YieldNestOracle} from "contracts/YieldNestOracle.sol";
import {ynBase} from "contracts/ynBase.sol";
...
contract ynLSD is IynLSD, ynBase, ReentrancyGuardUpgradeable, IynLSDEvents {
  ...
  bytes32 public constant LSD_STAKING_NODE_CREATOR_ROLE =
    keccak256("LSD_STAKING_NODE_CREATOR_ROLE");
  ...
  function initializeBoostrapDeposit(Init calldata init) internal {
    ...
    uint256 bootstrapAmount = BOOTSTRAP_AMOUNT_UNITS *
      (10 ** (IERC20Metadata(address(assets[0])).decimals()));
  ...
  function _convertToShares(
    ...
    return Math.mulDiv(amount, totalSupply(), totalAssets(), rounding);
  }
   ...
   */
  function getTotalAssets() public view returns (uint256[] memory assetBalances) {
   ...
   */
  function initializeLSDStakingNode(ILSDStakingNode node, uint256 nodeId) internal virtual {
    uint64 initializedVersion = node.getInitializedVersion();
    if (initializedVersion == 0) {
      node.initialize(ILSDStakingNode.Init(IynLSD(address(this)), nodeId));
   ...
   */
  function registerLSDStakingNodeImplementationContract(
   ...
   */
  function upgradeLSDStakingNodeImplementation(
    address _implementationContract
  ) public onlyRole(STAKING_ADMIN_ROLE) notZeroAddress(_implementationContract) {
    if (address(upgradeableBeacon) == address(0)) {
